...
const objectsService = {
     ...
     * @async
     * @function
     * @param {ObjectBoundary} reqObjectBoundary - The user details to create a new user.
    ...
    /**
     * retrieve the children array of an object.
     ...
     * @param {string} userEmail - The email of the user making the request.
     * @param {string} userPlatform - The platform of the user making the request.
     * @param {ObjectIdBoundary} objectIdBoundary - The object id boundary of the child object.
     * @returns {undefined}
     * @throws {Error} Throws an error if the request encounters any issues.
     */
    bindNewChild: async (
    ...
    getSpecificObjectByTypeAndName: async (targetType, objectName, userEmail, userPlatform) => {
        const existingUser = await UserModel.findOne({
            userId: userEmail + "$" + userPlatform,
        });
