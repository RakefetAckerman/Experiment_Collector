...
const commandsController = {
  ...
  createCommand: async (req, res) => {
    try {
      ...
      const errorMessage = process.env.NODE_ENV !== 'prod' ? error.message : 'An error occurred during command creation.';
  ...
  getAllCommands: async (req, res) => {
    try {
      ...
      const errorMessage = process.env.NODE_ENV !== 'prod' ? error.message : 'An error occurred during command retrieval.';
      res.status(error.status || 500).json({ error: errorMessage });
    }
  },
  ...
  deleteAllCommands: async (req, res) => {
    ...
    try {
      ...
      const errorMessage = process.env.NODE_ENV !== 'prod' ? error.message : 'An error occurred during command deletion.';
      res.status(error.status || 500).json({ error: errorMessage });
    }
  }
};
