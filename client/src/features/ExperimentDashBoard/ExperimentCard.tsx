...
function ExperimentCard({to, headline, image, text , className}: ExperimentCardProps) {
    const transition = "active:scale-110 transition-all duration-200 hover:bg-buttons-blue"

    return (
        <NavLink to={to}
                 className={`${transition} gap-7 p-5 w-96 min-h-40 bg-white drop-shadow-lg items-center justify-center flex flex-row rounded-3xl  ${className}`}>
