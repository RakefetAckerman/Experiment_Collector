...
function ExperimentPage() {
    ...
    if (error || !experimentData) {
        return <div className={"w-full h-full flex justify-center items-center flex-col gap-0 "}>
            <div
                className={"bg-white p-10 rounded-3xl drop-shadow-lg border gap-3 border-red-400 flex justify-center items-center flex-col"}>
                <h2 className={"text-clamping-mid font-exo text-center"}>Error occurred while getting Data</h2>
                <NavLink to={"/"}
                         className={"text-clamping-sm text-center hover:opacity-100 opacity-30 duration-200 transition-all"}>Return
                    Home</NavLink>
            </div>
        </div>
    }
    if (experimentData && !experimentData!.trialTypes[currentTrailType]) {
        return <div className={"w-full h-full flex justify-center items-center flex-col gap-0 "}>
            ...
            >
                <h2 className="text-white text-clamping-mid font-exo text-center">Thank You for participating in the
                    experiment</h2>
                <NavLink
                    to="/"
