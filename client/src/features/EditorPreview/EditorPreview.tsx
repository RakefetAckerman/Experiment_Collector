...
import {isItemTypeEditor} from "../TrialType/types.ts";
import {getRandomItem} from "../../utils/helperMethods.ts";
...
function EditorPreview() {
    ...
    const index = useSelector((state: EditorState) => (state.editor.indexInPreview));
    if (!experimentData || !item) {
        return null;
    }
    let itemForRenderer = item;
    if (index && items[index]) {
        const currentItem = items[index];
        itemForRenderer = isItemTypeEditor(currentItem) ? currentItem : getRandomItem(currentItem.items);
    }
    ...
    return (
        <div
            ...
            {item && <EditorItemRenderer startTime={Date.now()} item={itemForRenderer}/>}
        </div>
    );
}
