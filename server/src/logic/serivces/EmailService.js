    ...
    sendVerificationCode: async (emailDest) => {
        try {
            if (!emailDest) {
                ...
                throw new createHttpError.BadRequest("The email for sending verification code is invalid");
            }
            ...
            switch(error.code){
                case 'EENVELOPE':
                    ...
                    error = createHttpError.BadRequest(`The email ${emailDest} for sending verification code is invalid`)
                    break;
                case 'ENOTFOUND':
                    logger.error(`SMTP server not found. Please check your email server settings.`);
                    error =createHttpError.BadRequest(`There is no available SMTP server for verification at the moment`)
                    break;
                default:
